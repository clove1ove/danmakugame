...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const [bossApproaching, setBossApproaching] = useState(true);
  ...
  const handleBossHit = (damage: number) => {
    ...
    const player = playerRef.current;
    if (bossApproaching) return; // 登場中は無敵
  ...
  const fireBossBullets = () => {
    ...
    if (boss.phase === 1) {
      ...
      if (timer % Math.max(8, Math.floor(baseFreq / boss.angerFactor)) === 0) {
        ...
        const streams = difficulty === 'LUNATIC' ? 6 : difficulty === 'HARD' ? 4 : 3;
    ...
    // --- PHASE 2: 雷鳴の叫び & 高速雷撃雷雨 ---
      ...
      if (timer % Math.max(45, Math.floor(aimFreq / boss.angerFactor)) === 0) {
        ...
        // 難易度によってWay数が変わる
        const count = difficulty === 'EASY' ? 3 : (difficulty === 'NORMAL' ? 5 : 7);
        const spread = 0.12; // 弾同士の間隔（ラジアン）
    ...
    // --- PHASE 3: 最終奥義「百鬼夜行・怨嗟の嵐」 ---
      ...
      if (timer % Math.max(25, Math.floor(waveFreq / boss.angerFactor)) === 0) {
        SoundManager.playEnemyShoot(false);
        const count = difficulty === 'EASY' ? 16 : (difficulty === 'NORMAL' ? 24 : (difficulty === 'HARD' ? 32 : 44));
        // 時計回りと反時計回りが交互になる
  ...
  const updateGameState = () => {
    ...
    if (bossApproaching) {
      ...
      // 普段は左右にふわふわ滑らかに浮遊移動
      boss.moveTimer += 0.015;
      boss.x = WIDTH / 2 + Math.sin(boss.moveTimer * 0.6) * 110;
      boss.y = boss.targetY + Math.cos(boss.moveTimer * 1.1) * 15;
    ...
    if (player.shootCooldown <= 0 && !bossApproaching) {
  ...
  const renderGame = () => {
    ...
    if (!bossApproaching) {
      // 鬼の後ろに流れる妖艶な赤/青の円形グラデーション（邪悪なオーラ）
      ctx.save();
    ...
    if (boss.phase >= 2 && !bossApproaching) {
